...
def plot_loss(epoch):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('images/WGAN_Dloss_epoch_%d.png' % epoch)
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('images/WGAN_Gloss_epoch_%d.png' % epoch)
...
def plotGeneratedImages(epoch, example=100, dim=(10, 10), figsize=(10, 10)):
    ...
    if not os.path.exists('images/WGAN'):
        os.mkdir('images/WGAN')
    plt.savefig('images/WGAN/epoch_%d.png' % epoch)


def saveModels(epoch):
    if not os.path.exists('Model'):
        os.mkdir('Model')
    generator.save('Model/WGAN/generated_epoch_%d.h5' % epoch)
    discriminator.save('Model/WGAN/discriminated_epoch_%d.h5' % epoch)
